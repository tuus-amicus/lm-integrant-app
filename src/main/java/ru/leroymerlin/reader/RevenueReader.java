...
public class RevenueReader {
    public static Map<ReportType, BigDecimal> getRevenue() throws URISyntaxException, IOException {
        List<String> content = FileResourceUtil.getContentFromFile("billing/revenue.csv");

        return content.stream().skip(1)
                .map(it -> it.split(","))
                .map(it -> new Revenue(ReportType.valueOf(it[0]), BigDecimal.valueOf(Double.parseDouble(it[1]))))
